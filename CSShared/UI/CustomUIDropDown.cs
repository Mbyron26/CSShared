...
public class CustomUIDropDown : CustomUIDropDownBase<CustomUIListBox> {
    ...
    public static CustomUIDropDown AddCPDropDown(UIComponent parent, Vector2 size, string[] items, int selectedIndex, Action<int> callback1 = null, Action<string> callback2 = null, Action<int, string> callback3 = null) {
        ...
        dropDown.EventPopupAdded += (dropDown, listBox) => {
            listBox.canFocus = true;
            listBox.TextScale = 0.8f;
            listBox.TextAtlas = listBox.FgAtlas = listBox.BgAtlas = CustomUIAtlas.CSSharedAtlas;
            ...
            listBox.ItemHeight = (int)dropDown.height;
            listBox.TextHorizontalAlignment = UIHorizontalAlignment.Left;
        };
        dropDown.TextAtlas = dropDown.FgAtlas = dropDown.BgAtlas = CustomUIAtlas.CSSharedAtlas;
        ...
        dropDown.EventSelectedIndex1Changed += (c, v) => callback1?.Invoke(v);
        dropDown.EventSelectedIndex2Changed += (c, v) => callback2?.Invoke(v);
        dropDown.EventSelectedIndex3Changed += (c, v, r) => callback3?.Invoke(v, r);
        dropDown.RenderFg = true;
        return dropDown;
    }
    public static CustomUIDropDown AddOPDropDown(UIComponent parent, Vector2 size, string[] items, int selectedIndex, Action<int> callback = null, Action<string> callback2 = null, Action<int, string> callback3 = null) {
        var dropDown = parent.AddUIComponent<CustomUIDropDown>();
        dropDown.size = size;
        dropDown.ListHeight = (int)(size.y * items.Length + 8);
        dropDown.TextHorizontalAlignment = UIHorizontalAlignment.Left;
        ...
        dropDown.items = items;
        dropDown.SelectedIndex = selectedIndex;
        dropDown.EventSelectedIndex1Changed += (c, v) => callback?.Invoke(v);
        dropDown.EventSelectedIndex2Changed += (c, v) => callback2?.Invoke(v);
        dropDown.EventSelectedIndex3Changed += (c, v, r) => callback3?.Invoke(v, r);
        dropDown.RenderFg = true;
        return dropDown;
...
public abstract class CustomUIDropDownBase<T> : CustomUITextComponent where T : CustomUIListBox {
    ...
    public event Action<CustomUIDropDownBase<T>, int> EventSelectedIndex1Changed;
    public event Action<CustomUIDropDownBase<T>, string> EventSelectedIndex2Changed;
    public event Action<CustomUIDropDownBase<T>, int, string> EventSelectedIndex3Changed;
    ...
    public virtual int SelectedIndex {
        ...
        set {
            ...
            if (value != selectedIndex) {
                ...
                selectedIndex = value;
                EventSelectedIndex1Changed?.Invoke(this, SelectedIndex);
                EventSelectedIndex2Changed?.Invoke(this, SelectedValue);
                EventSelectedIndex3Changed?.Invoke(this, SelectedIndex, SelectedValue);
                Invalidate();
            }
        }
    }
